...
// SLA-backed — this tries litterbox first, falls back to tmpfiles.org if
// it fails or hangs, so a single flaky third party can't take down PDF
// delivery on its own.
// ═══════════════════════════════════════════════════════
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/api/sentwa/send-document') {
    try {
      ...
      // Step 1 — upload to a short-lived public host so SentWA can fetch it.
      // Anonymous free hosts are convenient (no account/API key needed) but
      // not fully reliable on their own — litterbox has hung/failed
      // intermittently during testing. Two independent providers race in
      // TRUE parallel (Promise.any — resolves the instant the first one
      // succeeds, doesn't wait for a slower loser to also finish) so a
      // slow/hanging provider can't add its full timeout on top of a fast
      // one that already worked. A sequential try-then-fallback would have
      // meant every request eats the first provider's full timeout even
      // when it's doomed to fail fast — that stacked latency, with zero UI
      // feedback while it ran, is what made a real send feel "frozen."
      ...
      // Log each failure as it happens, independent of the race outcome —
      // attaching a .catch() here doesn't affect Promise.any's own view of
      // whether that same promise rejected.
      attempts.forEach((p, i) => p.catch(e => console.log(`[sentwa-proxy] PDF upload attempt failed — ${providers[i].name}: ${e.message}`)));
      ...
      if (uploadedUrl) console.log(`[sentwa-proxy] Uploaded PDF via ${winner.provider}: ${uploadedUrl}`);
      if (!uploadedUrl) {
        sendJson(res, 502, { ok: false, error: 'PDF upload failed on all providers', detail: uploadErrors.join(' | ') });
        return;
      }

      // Step 2 — send via SentWA as a real document attachment.
      const params = new URLSearchParams({
        number,
        type: 'document',
      ...
      const text = await upstream.text();
      console.log(`[sentwa-proxy] Document send response ${upstream.status}: ${text.slice(0, 300)}`);
